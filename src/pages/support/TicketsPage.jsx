...
import { listTickets, assignTicket, addTicketNote, resolveTicket, escalateTicket } from '../../api/support'
...
const IconUser = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
...
export default function TicketsPage() {
  ...
  const myId = decodeJwtPayload(auth?.token)?.id
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50/70 border-b border-slate-100">
              <tr>
                {['Subject', 'Raised By', 'Priority', 'Status', 'Assigned To', 'Created', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={t._id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${PRIORITY_CLS[t.priority]}`}>{t.priority}</span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleAssignToMe(t._id)} disabled={busyId===t._id} className="w-7 h-7 flex items-center justify-center rounded-lg bg-info/8 text-info hover:bg-info/15 disabled:opacity-50" title="Assign to me"><IconUser /></button>
